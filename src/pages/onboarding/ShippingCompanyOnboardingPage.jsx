...
import ShippingCompanyStep1 from '../../components/onboarding/shippingCompany/ShippingCompanyStep1';
import ShippingCompanyStep2 from '../../components/onboarding/shippingCompany/ShippingCompanyStep2';
import ShippingCompanyStep3 from '../../components/onboarding/shippingCompany/ShippingCompanyStep3';
import logo from '../../assets/CIQ Logo 1.png';

const ShippingCompanyOnboardingPage = () => {
  const navigate = useNavigate();
  const dispatch = useDispatch();
  const { user } = useSelector((state) => state.auth);
  const { showSuccess, showError } = useAlert();

  // Initialize current step based on shippingCompanyFormCompleted
  const getInitialStep = () => {
    const completed = user?.shippingCompanyFormCompleted || 0;
    return completed + 1;
  };
  ...
  }, [user?.shippingCompanyFormCompleted]);
  ...
  const handleStep1Next = (data) => {
    ...
    dispatch(setCredentials({
      ...
      user: { ...user, shippingCompanyFormCompleted: 1 }
    }));
    setCurrentStep(2);
  };
  ...
  const handleStep2Next = (data) => {
    ...
    dispatch(setCredentials({
      token: localStorage.getItem('token'),
      user: { ...user, shippingCompanyFormCompleted: 2 }
    }));
    setCurrentStep(3);
  };
  ...
  const handleStep3Submit = async (data) => {
    ...
    // try {
    //   const response = await completeShippingCompanyOnboarding(finalData).unwrap();
    //   dispatch(setCredentials({
    //     token: response.token,
    //     user: { ...response.user, onboardingCompleted: true }
    //   }));
    //   showSuccess('Shipping company onboarding completed successfully!');
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div className="border-l border-gray-300 pl-4">
                <h1 className="text-lg font-bold text-gray-900">Shipping Company Onboarding</h1>
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-4xl mx-auto px-8 py-6">
          <div className="flex items-center justify-between mb-2">
            {[
              { num: 1, label: 'Vessel Operations' },
              { num: 2, label: 'Compliance & Integration' },
...
export default ShippingCompanyOnboardingPage;
