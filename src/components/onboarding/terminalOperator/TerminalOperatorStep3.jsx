...
const TerminalOperatorStep3 = ({ onSubmit, onBack, initialData, isLoading }) => {
  const [uploadFile] = useUploadFileMutation();
  const { showSuccess, showError } = useAlert();

  const [formData, setFormData] = useState({
    npaLicense: initialData?.npaLicense || null,
    terminalOperatingLicense: initialData?.terminalOperatingLicense || null,
    customsClearancePermit: initialData?.customsClearancePermit || null,
    insuranceCertificate: initialData?.insuranceCertificate || null,
    safetyComplianceCertificate: initialData?.safetyComplianceCertificate || null,
    ...initialData,
  });
  ...
  const documentFields = [
    {
      name: 'npaLicense',
      label: 'NPA Terminal License',
      folder: 'terminal-licenses',
      required: true,
      description: 'Upload your Nigerian Ports Authority terminal operating license',
    },
    {
      name: 'terminalOperatingLicense',
      label: 'Terminal Operating License',
      folder: 'terminal-licenses',
      required: true,
      description: 'Upload current terminal operating license from relevant authority',
    },
    {
      name: 'customsClearancePermit',
      label: 'Customs Clearance Permit',
      folder: 'terminal-licenses',
      required: true,
      description: 'Upload customs clearance permit (if terminal provides customs services)',
    },
    {
      name: 'insuranceCertificate',
      label: 'Terminal Liability Insurance',
      folder: 'terminal-licenses',
      required: true,
      description: 'Upload current terminal liability and cargo insurance certificate',
    },
    {
      name: 'safetyComplianceCertificate',
      label: 'Safety & Environmental Compliance',
      folder: 'terminal-licenses',
      required: false,
      description: 'Upload safety and environmental compliance certificates (ISPS, ISO, etc.)',
    },
  ];
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        ...
        <ul className="text-xs text-blue-700 space-y-1 list-disc list-inside">
          ...
          <li>Upon approval, you'll have full access to gate operations, yard management, and reporting features</li>
          <li>You can start integrating ContainerIQ with your gate systems and TMS immediately after approval</li>
          <li>Real-time dwell time monitoring and alerts will be activated upon integration</li>
        </ul>
      </div>
...
export default TerminalOperatorStep3;
